...
import namesIcon from "./assets/names.png";
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-cover bg-center bg-no-repeat flex" style={{ backgroundImage: `url(${bg})` }}>
        ...
        {/* Names List */}
        <div className="w-[25%] p-6 bg-[#1a1a1a] backdrop-blur-md border-r border-white/20">
          <div className="mb-6">
            <div className="flex items-center mb-10 mt-5">
              <span><img src={namesIcon} alt="" className="w-[65px]" /></span>
              <h2 className="text-[1.9rem] font-bold text-white m-auto">Participants</h2>
              <span className="bg-white/20 text-white px-2 py-1 rounded-full text-[1rem]">
                {names.length}
              </span>
            </div>
          </div>
          ...
          {loadingData ? (
            <p className="text-white/60">Fetching names...</p>
          ) : (
            <NamesList
              names={names}
              winner={winner}
